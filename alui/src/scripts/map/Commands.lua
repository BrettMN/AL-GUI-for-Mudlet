...
function map.show_help()
    ...
    echo("    2) Reconciles connected exits: walks the exit graph and gently moves rooms whose coordinates\n")
    echo("       disagree with their exit offsets, skipping rooms that can't move due to collisions.\n")
    echo("    3) Flattens cardinally connected rooms to the current room's z-level.\n")
    echo("    4) Snaps in-area up/down room pairs to exact ±1 z-offsets (fixes sky-room horizontal drift).\n")
    echo("    5) Reports remaining anomalies by category (cyclic mismatches, shared-target bugs, etc.).\n")
    ...
    echo("    2) Rebuilds all room coordinates from scratch via BFS from the current room.\n")
    ...
    echo("    3) Underground rooms (caves, tunnels) are placed on a separate z-level automatically.\n")
